# SM3D — библиотека расчёта солнечного давления
...
* **точные оптические свойства** для каждой грани (коэффициент отражения ρ,
...
* $\alpha = $ `reflectance` (ρ), $\mu = $ `specularity` (s).
...
struct Triangle {
    ...
    double reflectance = 0.0;   // ρ ∈ [0,1] — доля отражённого потока
    double specularity = 0.0;   // s ∈ [0,1] — доля зеркального отражения
